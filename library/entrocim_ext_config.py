...
def main():
    module = AnsibleModule(
        argument_spec = dict(
           entrocim_ext_name= dict(required=True, type='str'),
           entrocim_hostname = dict(required=True, type='str'),
           entrocim_ext_state=dict(default='add',type='str', choices=['add','remove']),
           entrocim_ext_project=dict(required=True, type='str')
        ),
        supports_check_mode=True
    )
    h = openHaystackConnection(module.params['entrocim_hostname'],module.params['entrocim_ext_project'])
    state = module.params['entrocim_ext_state']
    if state == "add":
        result = h.eval("extAdd([\"" + module.params['entrocim_ext_name'] + "\"])")
    elif state == "remove": 
        result = h.eval("extRemove([\"" + module.params['entrocim_ext_name'] + "\"])")
    else:
        module.fail_json(msg="State option invalid " + module.params["entrocim_ext_state"] )

    if "err" in result: 
        module.fail_json(msg=result)
    else: 
        module.exit_json(changed=False, stdout="Ext" + module.params["entrocim_ext_name"] + " enabled")
